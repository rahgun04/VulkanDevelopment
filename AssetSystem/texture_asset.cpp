...
assets::TextureInfo assets::read_texture_info(AssetFile* file)
{
	...
	for (auto& el : texture_metadata["pages"].items())
	{
		PageInfo page;

		page.compressedSize = el.value()["compressed_size"];
		page.originalSize = el.value()["original_size"];
		page.width = el.value()["width"];
		page.height = el.value()["height"];
