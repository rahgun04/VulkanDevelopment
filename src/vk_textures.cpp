...
bool vkutil::load_image_from_asset(VulkanEngine& engine, const char* filename, AllocatedImage& outImage)
{
	...
	imageExtent.width = static_cast<uint32_t>(textureInfo.pages[0].width);
	imageExtent.height = static_cast<uint32_t>(textureInfo.pages[0].height);
	imageExtent.depth = 1;
	...
	engine._mainDeletionQueue.push_function([=, &engine]() {
		...
		vmaDestroyImage(engine._allocator, newImage._image, newImage._allocation); //Dodgy Delete
		});
