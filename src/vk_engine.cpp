...
do { \
    ...
    { \
        ...
        std::cout <<"Detected libOVR error: " << std::string(info.ErrorString) << std::endl; \
        abort(); \
    } \
} while (0)
...
void VulkanEngine::init_oculus() {
	...
	auto inst_ret = builder.set_app_name("Example VR Vulkan Application")
	...
	_session = session;
	_luid = luid;
	ovr_GetSessionPhysicalDeviceVk(_session, _luid, _instance, &_chosenGPU);
	...
	vkGetPhysicalDeviceQueueFamilyProperties(_chosenGPU, &queueFamilyCount, queueFamilyProps);

	_graphicsQueueFamily = queueFamilyCount;

	for (uint32_t i = 0; i < queueFamilyCount; ++i)
	{
		VkBool32 presentable = false;
		CHECKVK(vkGetPhysicalDeviceSurfaceSupportKHR(_chosenGPU, i, _surface, &presentable));
	...
	CHECK(_graphicsQueueFamily < queueFamilyCount);
	...
	create_oculus_renderTarget();
	
	//everything went fine
	_isInitialized = true;
}

void VulkanEngine::init_oculus_frameBuffer(RenderTarget* rt) {
	//create the framebuffers for the swapchain images. This will connect the render-pass to the images for rendering
	VkFramebufferCreateInfo fb_info = {};
	fb_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
	fb_info.pNext = nullptr;

	fb_info.renderPass = rt->rp;
	fb_info.attachmentCount = 1;
	fb_info.width = _windowExtent.width; //TODO: Manage Size
	fb_info.height = _windowExtent.height;
	fb_info.layers = 1;

	//grab how many images we have in the swapchain
	const uint32_t swapchain_imagecount = rt->tex.rendTex.size();
	...
	for (int i = 0; i < swapchain_imagecount; i++) {
		...
		attachments[0] = rt->tex.rendTex[i].colView;
		attachments[1] = rt->tex.rendTex[i].depthView;

		fb_info.pAttachments = attachments;
		fb_info.attachmentCount = 2;

		VK_CHECK(vkCreateFramebuffer(_device, &fb_info, nullptr, &rt->tex.rendTex[i].fb));

		_mainDeletionQueue.push_function([=]() {
			vkDestroyFramebuffer(_device, rt->tex.rendTex[i].fb, nullptr);
			vkDestroyImageView(_device, rt->tex.rendTex[i].colView, nullptr);
			vkDestroyImageView(_device, rt->tex.rendTex[i].depthView, nullptr);
		});
	}
}

void VulkanEngine::init_oculus_pipelines(RenderTarget* rt)
{
...
//we are not using descriptor sets or other systems yet, so no need to use anything other than empty default
	...
	pipelineBuilder._viewport.width = (float)rt->size.width; //TODO: Sort out Size
	pipelineBuilder._viewport.height = (float)rt->size.height;
	pipelineBuilder._viewport.minDepth = 0.0f;
	pipelineBuilder._viewport.maxDepth = 1.0f;

	pipelineBuilder._scissor.offset = { 0, 0 };//TODO: Sort out Size
	pipelineBuilder._scissor.extent = rt->size;
...
void VulkanEngine::oculus_load_meshes()
{
...
void VulkanEngine::create_oculus_renderTarget() {
	for (auto eye : { ovrEye_Left, ovrEye_Right }){
		//TODO: instanciate Render Target classes 
		
		ovrSizei s = ovr_GetFovTextureSize(_session, eye, hmdDesc.DefaultEyeFov[eye], 1);
		eyes[eye] = RenderTarget(s);

		create_oculus_swapchain(&eyes[eye]);
		init_oculus_commands(&eyes[eye]);
		init_oculus_default_renderpass(&eyes[eye]);
		init_oculus_frameBuffer(&eyes[eye]);
		init_oculus_pipelines(&eyes[eye]);
		oculus_load_meshes();
		init_oculus_scene();
	}
}
//TODO
void VulkanEngine::create_oculus_swapchain(RenderTarget* rt) {
	// depth
	ovrTextureSwapChainDesc depthDesc = {};
	depthDesc.Type = ovrTexture_2D;
	depthDesc.ArraySize = 1;
	depthDesc.Format = OVR_FORMAT_D32_FLOAT;
	depthDesc.Width = (int)rt->size.width;//TODO: Manage Size
	depthDesc.Height = (int)rt->size.height;
	...
	CHECKOVR(ovr_CreateTextureSwapChainVk(_session, _device, &depthDesc, &rt->tex.depthSwapChain));
	...
	colorDesc.Width = (int)rt->size.width;//TODO: Manage Size
	colorDesc.Height = (int)rt->size.height;
	...
	CHECKOVR(ovr_CreateTextureSwapChainVk(_session, _device, &colorDesc, &rt->tex.depthSwapChain));

	_mainDeletionQueue.push_function([=]() {
		ovr_DestroyTextureSwapChain(_session, rt->tex.colSwapChain);
		
		ovr_DestroyTextureSwapChain(_session, rt->tex.depthSwapChain);
		});

	int textureCount = 0;
	CHECKOVR(ovr_GetTextureSwapChainLength(_session, rt->tex.colSwapChain, &textureCount));

	int depthCount = 0;
	CHECKOVR(ovr_GetTextureSwapChainLength(_session, rt->tex.depthSwapChain, &depthCount));
	...
	{
		rt->tex.rendTex.push_back(RenderTexture());
		VkImage colorImage;
		...
		rt->tex.rendTex[i].colImg = colorImage;
		rt->tex.rendTex[i].depthImg = depthImage;


		_mainDeletionQueue.push_function([=]() {
			vkDestroyImage(_device, rt->tex.rendTex[i].colImg, nullptr);
			vkDestroyImage(_device, rt->tex.rendTex[i].depthImg, nullptr);
		});

		// Create color image view
		VkImageViewCreateInfo colorViewInfo = { VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO };
		colorViewInfo.image = rt->tex.rendTex[i].colImg;
		...
		CHECKVK(vkCreateImageView(_device, &colorViewInfo, nullptr, &rt->tex.rendTex[i].colView));

		// Create depth image view
		VkImageViewCreateInfo depthViewInfo = { VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO };
		depthViewInfo.image = rt->tex.rendTex[i].depthImg;
		...
		CHECKVK(vkCreateImageView(_device, &depthViewInfo, nullptr, &rt->tex.rendTex[i].depthView));
...
void VulkanEngine::init_oculus_commands(RenderTarget* rt)
{
	...
	for (int i = 0; i < FRAME_OVERLAP; i++) {
		...
		VK_CHECK(vkCreateCommandPool(_device, &commandPoolInfo, nullptr, &get_current_oculus_frame()._commandPool));

		//allocate the default command buffer that we will use for rendering
		VkCommandBufferAllocateInfo cmdAllocInfo = vkinit::command_buffer_allocate_info(get_current_oculus_frame()._commandPool, 1);

		VK_CHECK(vkAllocateCommandBuffers(_device, &cmdAllocInfo, &get_current_oculus_frame()._mainCommandBuffer));

		_mainDeletionQueue.push_function([=]() {
			vkDestroyCommandPool(_device, get_current_oculus_frame()._commandPool, nullptr);
		});
	}
}
...
void VulkanEngine::init_oculus_scene()
{
	RenderObject monkey;
...
void VulkanEngine::init_oculus_default_renderpass(RenderTarget* rt) {
	// the renderpass will use this color attachment.
	VkAttachmentDescription color_attachment = {};
	//the attachment will have the format needed by the swapchain
	color_attachment.format = _swapchainImageFormat;
	...
	VK_CHECK(vkCreateRenderPass(_device, &render_pass_info, nullptr, &rt->rp));


	_mainDeletionQueue.push_function([=]() {
		vkDestroyRenderPass(_device, rt->rp, nullptr);
		});
}
...
void VulkanEngine::oculus_cleanup() //TODO:
{
	if (_isInitialized) {
...
void VulkanEngine::oculus_draw()
{
	...
	for (auto eye : { ovrEye_Left, ovrEye_Right })
		eyeRenderDesc[eye] = ovr_GetRenderDesc(_session, eye, hmdDesc.DefaultEyeFov[eye]);
	...
	ovr_GetEyePoses(_session, _frameNumber, ovrTrue, HmdToEyePose, eyeRenderPose, &sensorSampleTime);
	...
	VK_CHECK(vkResetCommandBuffer(get_current_oculus_frame()._mainCommandBuffer, 0));
	//naming it cmd for shorter writing
	VkCommandBuffer cmd = get_current_oculus_frame()._mainCommandBuffer;
	...
	for (auto eye : { ovrEye_Left, ovrEye_Right })
	{
		...
		ovr_GetTextureSwapChainCurrentIndex(_session, eyes[eye].tex.colSwapChain, &swapchainImageIndex);

		VkRenderPassBeginInfo rpInfo = vkinit::renderpass_begin_info(eyes[eye].rp, eyes[eye].size, eyes[eye].tex.rendTex[swapchainImageIndex].fb); //TODO Size
		...
		OVR::Matrix4f proj = OVR::Matrix4f::Scaling(1.0f, -1.0f, 1.0f) * ovrMatrix4f_Projection(hmdDesc.DefaultEyeFov[eye], 0.2f, 1000.0f, ovrProjection_None);
...
//we will signal the _renderSemaphore, to signal that rendering has finished
	...
	for (auto eye : { ovrEye_Left, ovrEye_Right })
	{
		ovr_CommitTextureSwapChain(_session, eyes[eye].tex.colSwapChain);
		ovr_CommitTextureSwapChain(_session, eyes[eye].tex.depthSwapChain);
	}
	...
	for (auto eye : { ovrEye_Left, ovrEye_Right })
	{
		ld.ColorTexture[eye] = eyes[eye].tex.colSwapChain;
		ld.DepthTexture[eye] = eyes[eye].tex.depthSwapChain;
		ld.Viewport[eye] = eyes[eye].tex.GetViewport();
		ld.Fov[eye] = hmdDesc.DefaultEyeFov[eye];
		ld.RenderPose[eye] = eyeRenderPose[eye];
	}

	ovrLayerHeader* layers = &ld.Header;
	ovrResult result = ovr_SubmitFrame(_session, _frameNumber
		, nullptr, &layers, 1);
...
OculusFrameData& VulkanEngine::get_current_oculus_frame()
{
	return oculus_frames[_frameNumber % FRAME_OVERLAP];
}

OVR::Recti TextureSwapChain::GetViewport()
{
	return OVR::Recti(0, 0, size.width, size.height);
}

RenderTarget::RenderTarget(ovrSizei sizeIn)
{
	size.height = sizeIn.h;
	size.width = sizeIn.w;
...
void TextureSwapChain::init(VkExtent2D sizeIn) {
	size = sizeIn;
	rendTex = vector<RenderTexture>();
}
